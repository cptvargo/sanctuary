...
import ScriptureEditor, { AnnouncementEditor } from '../slides/editors/ScriptureEditor'
...
function badgeColorFor(section) {
  ...
  const map = { C:'#ff9500',C1:'#ff9500',C2:'#ff9500',C3:'#ff9500',B:'#bf5af2',B2:'#bf5af2',PC:'#30d158',I:'#636366',O:'#636366',T:'#30d158',H:'#30d158' }
...
function slidesToText(slides) {
  ...
  for (const s of slides) {
    if (s.section && s.section !== lastSection) { parts.push(`[${s.section}]`); lastSection = s.section }
...
function SongTextEditor({ item }) {
  ...
  useEffect(() => { setName(item.name); setText(slidesToText(item.slides)) }, [item.id])
  useEffect(() => { setPreview(parseSongText(text, name)) }, [text, name])
    ...
    <div className={styles.songEditorWrap}>
      ...
      {showMyImages && (
        <CustomImageManager
          ...
          onSelect={(img) => {
            const themeChanges = { bgImageUrl: img.dataUrl, bgGradient: null, bgColor: '#000000', bgOverlayOpacity: 0.50, textColor: '#ffffff', smartMediaId: img.id, fontId: 'montserrat' }
            useSanctuaryStore.setState(state => ({
              serviceOrder: state.serviceOrder.map(i =>
      ...
      {item.slides.map((slide) => {
        ...
        return (
          <div key={slide.id}
            ...
            <div className={styles.tileCanvas}><SlideCanvas slide={slide} mini /></div>
            <div className={styles.tileLabel}>
              ...
              {!isLiveSlide && badge && (
                <span className={styles.tileSectionBadge} style={{ background: badgeColorFor(slide.section) }}>{badge}</span>
              )}
            </div>
          </div>
        )
...
function StandaloneEditor({ item }) {
  const { updateSlide, liveSlideId, isLive, mode, serviceOrder } = useSanctuaryStore()
  ...
  const imgs = slide.type === 'image'
    ? (slide.images?.length > 0 ? slide.images : slide.imageDataUrl ? [{}] : [])
    : []
    ...
    <div className={styles.standaloneWrap}>
      ...
      {imgs.length > 1 && (
        <div className={styles.pptxNavBar}>
          <button className={styles.pptxNavBtn}
            onClick={() => { const n = Math.max(0, imgIdx-1); updateSlide(slide.id, { currentIndex: n }); if (isLive) useSanctuaryStore.getState()._syncProjector() }}
            disabled={imgIdx === 0}>‹ Prev</button>
          <span className={styles.pptxNavInfo}>{imgIdx + 1} of {imgs.length}</span>
          <button className={styles.pptxNavBtn}
            onClick={() => { const n = Math.min(imgs.length-1, imgIdx+1); updateSlide(slide.id, { currentIndex: n }); if (isLive) useSanctuaryStore.getState()._syncProjector() }}
            disabled={imgIdx >= imgs.length - 1}>Next ›</button>
        </div>
      )}
    ...
    <div className={styles.panel}>
      <div className={styles.itemHeader}>
        ...
        <div className={styles.itemActions}>
          {isClickToSend && <span className={styles.liveHint}>● Preview — click any slide to send live</span>}
          {isSong && isEditMode && <span className={styles.editingBadge}>✎ Editing — changes auto-save</span>}
        </div>
      </div>
        ...
        <div className={styles.tileEditorBar}>
          ...
          {(selectedSlide.bgImageUrl || (selectedSlide.smartMediaId && selectedSlide.smartMediaId !== 'none')) && (
            <>
              ...
              <span className={styles.tileEditorVal}>{Math.round((selectedSlide.bgOverlayOpacity ?? 0.55) * 100)}%</span>
              <span className={styles.tileEditorDivider} />
            </>
          )}
