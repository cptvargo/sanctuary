...
export default function LogoEditor({ slide, onChange }) {
  const { churchName = '', tagline = '', bgColor = '#000000', textColor = '#c8a84a', logoDataUrl } = slide
  ...
  return (
    <div className={styles.editor}>
      <div className={styles.row}>
        ...
        <input className={styles.input} value={churchName} onChange={e => onChange({ churchName: e.target.value })} placeholder="Church name" />
      </div>
      <div className={styles.row}>
        <label className={styles.label}>Tagline</label>
        <input className={styles.input} value={tagline} onChange={e => onChange({ tagline: e.target.value })} placeholder="City, State or motto" />
      </div>
      <div className={styles.row}>
        ...
        {logoDataUrl && (
          <button className={styles.resetBtn} onClick={() => onChange({ logoDataUrl: null })}>Clear</button>
        )}
      </div>
      <div className={styles.row}>
